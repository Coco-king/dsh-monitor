...
- 改动任何中文文案后,需在三个文案源中同步检查:客户端 `lib/client-src/i18n.js`、服务端 `lib/messages.js`、配置校验 `lib/store.js`(校验文案按 `patch.locale` 决定语言)。
...
- 计费页易混淆点:主行三个输入框是「基础价」;「空闲/高峰」由「峰谷」开关控制显隐(纯展示层,不影响数值/计费),deepseek 开头模型默认开启。
