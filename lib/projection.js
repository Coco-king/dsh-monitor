 ...
 * 按事件时刻(event.time)用当次的峰值/空闲档位逐次计费,保证会话徽章历史正确。
 */

import { z } from 'zod'
import { costOf, priceEntryFor, priceTableFor } from './pricing.js'
...
export function makeCostUsageProjection(ledger) {
  ...
  return {
    ...
    apply(state, event) {
      ...
      // 按事件时刻计费(峰值/空闲两档);价格表按当前生效币种选取(与账本 account 同口径)。
      const atMs = Number.isFinite(Number(event.time)) && Number(event.time) > 0 ? Number(event.time) : Date.now()
      const billed = costOf(buckets, priceEntryFor(state.model, priceTableFor(ledger.config)), atMs, peakConfig())
