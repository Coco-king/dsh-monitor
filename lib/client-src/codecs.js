...
function parsePrice(v, path) {
  ...
  if (v.peakEnabled !== undefined) out.peakEnabled = v.peakEnabled === true
  ...
  if (v.windows !== undefined && v.windows !== null && typeof v.windows === 'object' && !Array.isArray(v.windows)) {
    const norm = list => Array.isArray(list)
      ? list.filter(w => w !== null && typeof w === 'object').map(w => ({ start: needNum(w.start, path + '.windows.start'), end: needNum(w.end, path + '.windows.end') }))
      : []
...
function parseUsageSummary(v, path) {
  ...
  const windows = {
    today: v.windows?.today === undefined || v.windows.today === null ? null : parseUsageTotals(v.windows.today, path + '.windows.today'),
    month: v.windows?.month === undefined || v.windows.month === null ? null : parseUsageTotals(v.windows.month, path + '.windows.month'),
    all: v.windows?.all === undefined || v.windows.all === null ? null : parseUsageTotals(v.windows.all, path + '.windows.all'),
  }
  const timeZone = v.timeZone === undefined || v.timeZone === null || typeof v.timeZone !== 'object' || Array.isArray(v.timeZone)
    ? undefined
    : { offset: typeof v.timeZone.offset === 'string' ? v.timeZone.offset : '', name: typeof v.timeZone.name === 'string' ? v.timeZone.name : undefined }
  const lastSweepAt = typeof v.lastSweepAt === 'number' ? v.lastSweepAt : undefined
  const diagnostics = v.diagnostics === undefined || v.diagnostics === null || typeof v.diagnostics !== 'object' || Array.isArray(v.diagnostics)
    ? { lastUsageAt: undefined, unattributedRows: 0 }
    : {
      lastUsageAt: typeof v.diagnostics.lastUsageAt === 'number' ? v.diagnostics.lastUsageAt : undefined,
  ...
  return {
    totals, byDay, models, sessions,
    byProvider, byProject, activity, activityModels, windows, timeZone, lastSweepAt, diagnostics, providers,
  }
}
...
const usageSummaryCodec = codecOf(parseUsageSummary)
const usageQueryCodec = codecOf(v => {
  if (v === null || typeof v !== 'object' || Array.isArray(v)) fail('query', 'object')
  return {
    range: v.range === undefined || v.range === null ? undefined : {
      start: typeof v.range.start === 'string' ? v.range.start : undefined,
      end: typeof v.range.end === 'string' ? v.range.end : undefined,
    },
    providers: Array.isArray(v.providers) ? v.providers.filter(p => typeof p === 'string') : undefined,
    models: Array.isArray(v.models) ? v.models.filter(m => typeof m === 'string') : undefined,
  }
})
...
export const CONTRIBUTION = {
  ...
  descriptors: [
    ...
    {
      id: 'dsh-monitor#monitor/getUsage', service: 'monitor', namespace: 'monitor', method: 'getUsage',
      invocation: { kind: 'direct' },
