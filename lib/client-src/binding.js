...
function startBinding(ctx, deps) {
  ...
  /** 从编辑按钮 aria-label「编辑 <显示名> (<id>)」/「Edit <显示名> (<id>)」解析提供方 id 与显示名。
   *  中英文各两态都视为「设置→模型」页的提供方编辑按钮:
   *   - 带括号 id:「编辑 DeepSeek (deepseek)」/「Edit DeepSeek (deepseek)」;
   *   - 显示名即 id 的裸 id:「编辑 deepseek」/「Edit deepseek」。
   *  其他页面(如 MCP 插件页的「编辑 MCP 服务器」/「Edit MCP server」)不命中,
   *  避免用量图标误注入到设置页其他分节的卡片上。 */
  const parseEditLabel = label => {
    ...
    // 无括号时的裸 id 必须是标识符样式(中英文下带空格/非标识符的都是其他页面的固定文案)。
