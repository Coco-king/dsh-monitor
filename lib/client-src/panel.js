...
function UsagePanel(props) {
  ...
  // OpenCode Go 内置:id 含 opencode 的未绑定提供方(含装插件前就配好的)无需
  // 先在设置里保存即可自动按 Go 订阅查询。与服务端 ensureUsage / 设置表单
  // 「按 ID 推导预设」同口径,大小写不敏感。
  const isBuiltinOpencode = typeof providerId === 'string' && /opencode/i.test(providerId)
  const preset = configured?.preset ?? (isBuiltinDeepseek ? 'deepseek' : isBuiltinOpencode ? 'opencode' : undefined)
...
function UsageButton(props) {
  ...
  // 生效语言实时取 store(宿主切换语言会写回 store,useMonitor 触发重渲染);
  // 注入的 t/locale 是注册时快照会过期,这里不再使用——与 SessionCost 同套取法。
  const t = makeT(storeSnap.locale ?? resolveLocale(config?.locale))
...
function SessionCost(props) {
  ...
  // 生效语言读 store(宿主语言写回 store.locale),与面板一致;缺失时回退配置解析。
  const locale = costStore?.locale ?? resolveLocale(config.locale)
