 ...
 */
function Dashboard({ wide, useMonitor, api, locale: baseLocale }) {
  const storeSnap = useMonitor ? useMonitor(s => s) : {}
  const config = storeSnap.config
  // 生效语言以 store 的 locale 为权威(宿主切换语言会实时写回),useMonitor 订阅触发
  // 重渲染;注入的 baseT/baseLocale 是注册时的快照,只在 store 缺省时兜底——
  // 与「会话费用角标」SessionCost 同一套取法,否则切英文后看板不跟随。
  const locale = storeSnap.locale ?? baseLocale
  const t = makeT(locale)
  const decimals = Math.max(0, Math.min(10, Math.floor(Number(config?.decimals) || 4)))
  const cny = activeCurrency(locale) === 'cny'
  const symbol = cny ? '¥' : '$'
  const money = {
