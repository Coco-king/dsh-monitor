...
function ActivityStrip({ data, t }) {
  ...
  return el(Fragment, null,
    ...
    hover === null ? null : el(DayTip, {
      ...
      models: modelsByDay.get(hover.cell.day) ?? [], clamp: hover.clamp, t,
    }))
}
...
function DayTip({ cell, x, y, level, models, t, clamp }) {
  ...
  // 靠边夹取:默认夹在画面内;悬停前记下的带区范围更准——最右列时把 tooltip 的
  // 右缘钳回带区可见区内,避免被面板/画面右缘截掉一角。
  const W = 250 // .dm-dash-tip 的 max-width
  const minX = clamp === null ? 8 : clamp.left + 8
  const maxX = clamp === null
    ? Math.max(8, window.innerWidth - W - 8)
    : Math.max(minX, clamp.right - W - 8)
