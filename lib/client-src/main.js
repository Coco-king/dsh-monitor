...
const { baseProviderId } = require('../provider-alias.js')
...
async function apply(ctx) {
  ...
  // ── 生效语言 ──────────────────────────────────────────────────────────
  // 宿主 DSH 的全局语言(LocaleRuntime,存 $DSH_HOME/settings.yaml locale.preference)
  // 是「设置→语言」的权威来源;不可用时回退到 resolveLocale(浏览器探测/配置)。
  const hostLocale = ctx.get('locale') // LocaleRuntime(LocaleFace getSnapshot/subscribe)
  const hostActive = () => {
    if (hostLocale !== undefined && typeof hostLocale.getLocale === 'function') {
  ...
  const effectiveLocale = () => {
    ...
    return resolveLocale(store.getSnapshot().config?.locale)
  }
  // 刷新生效语言到 store(触发订阅方);语言变化返回 true。
  ...
  // 把宿主语言写回 config.locale,服务端计费/文案按语言走(幂等:语言相等不再写)。
  const syncBackLocale = async () => {
    const snap = store.getSnapshot()
    const host = hostActive()
    if (host !== undefined && snap.config?.locale !== host) {
      try {
  ...
  // 宿主切换语言时:刷新生效语言 + 重注册设置分节 + 写回 config.locale。
  // (watch 注册放在 registerSection/sync 定义之后,见下方 saveStop。)
  ...
  /** 解析当前会话的模型提供方 id(读模型切换器自己的 service,不自己猜)。modlens 变体归一化回本体。 */
  const providerOf = sessionId => {
    ...
    try {
      ...
      // 变体已在设置绑定表单里显式配置时,尊重该配置(与服务端 ensureUsage 的
      // resolvedId 规则一致);否则回退到本体提供方 id。
      const configured = store.getSnapshot().config?.providers?.[raw] !== undefined
      return configured ? raw : baseProviderId(raw)
    } catch {
      return undefined
    }
  }
  const tOf = () => makeT(store.getSnapshot().locale)
  const injected = () => ({
  ...
  // 用量看板:侧边栏底部(设置齿轮旁,sidebar.footer.action 插槽)。
  // 徽标显示今日 token 总数;点击弹出浮动面板(统计卡/提供方分布/活跃度/模型表)。
  // 语言与币种由 Dashboard 从 store 实时取,这里只注入 api/store/locale(兜底),宿主追加 { wide }。
  ...
  if (hostLocale !== undefined && typeof hostLocale.subscribe === 'function') {
    stopLocaleWatch = ctx.effect(() => hostLocale.subscribe(() => {
      const changed = refreshStoreLocale()
      if (changed) sync() // 重注册设置分节(标签/面板随语言变化)
      void syncBackLocale()
    }), 'dsh-monitor: host locale watch')
  }

  return () => { stopSync(); if (stopLocaleWatch !== null) stopLocaleWatch() }
}
