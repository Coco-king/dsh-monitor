...
function ProviderForm(props) {
  ...
  return el('div', { className: 'dm-editor' },
    ...
    el('label', { className: 'dm-switch', style: { width: 'fit-content' } },
      ...
      el('span', { className: 'dm-switch-ui' }), t('enabled')),
...
function PricesSection(props) {
  ...
  // 某价格条目是否已自设窗口(任一档位非空即视为自设;自设后计费不再回退全局窗口)。
  const entryHasOwnWindows = entry => {
    const windows = entry?.windows
    return windows !== undefined && windows !== null
      && ((Array.isArray(windows.peak) && windows.peak.length > 0)
        || (Array.isArray(windows.offPeak) && windows.offPeak.length > 0))
  }
  // 某档位「生效窗口」:模型自设时按自设;未自设时高峰档回退到全局官方默认窗口
  // (与 tierFor 计费回退口径一致,让未点「从官方文档同步」时也能看到生效的高峰时段)。
  ...
  const setWindows = (modelId, tier, wins) => {
    setDraft(d => {
      ...
      const windows = { ...(entry.windows ?? {}) }
      // 首次为模型自设窗口时,把当前生效的官方默认高峰时段一并固化进该模型
      // (自设后计费不再回退全局),避免「高峰时段又变空」的困惑。
      if (wins.length > 0 && !entryHasOwnWindows(entry)
  ...
  const addWindow = () => {
    ...
    // 重叠校验与追加都以「生效窗口」为基准(未自设时含回退的官方默认高峰时段)。
  ...
  // 「把基础价填入空闲价」按钮:复制/重复图标(两张叠放的纸)而非下箭头——
  // 下箭头+横线读作「下载」,与别的下载按钮撞语义。
  const CopyIntoIcon = ({ size = 14 } = {}) => el('svg', { width: size, height: size, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg', 'aria-hidden': 'true' },
  ...
  // 某档位的时间窗口块:生效窗口标签 +「添加」按钮;点击后内联输入本地起止时间。
  // 模型未自设窗口时展示回退的官方默认高峰时段(只读,标记「官方默认」)。
  const tierWindowsBlock = (modelId, tier) => {
    ...
    const tag = (w, i) => el('span', { key: tier + ':' + i, className: 'dm-window-tag' + (inherited ? ' inherited' : '') },
      formatWindow(w),
      !inherited && el('button', {
      ...
      Object.keys(draft.models).map(modelId => {
        ...
        return el('div', { key: modelId, className: 'dm-price-row' },
          el('div', { className: 'dm-price-fields' },
            el('span', { className: 'dm-price-name', title: modelId },
            ...
            el('span', { className: 'dm-price-actions' },
              ...
              el('label', { className: 'dm-switch', title: t('tiersToggle') },
