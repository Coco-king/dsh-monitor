...
function PricesSection(props) {
  ...
  const addWindow = () => {
    ...
    const wins = [...windowsOf(winEdit.modelId, winEdit.tier), { start: localHourToUtc(from), end: localHourToUtc(to) }]
  ...
  const tierWindowsBlock = (modelId, tier) => {
    ...
    return el('div', { className: 'dm-tier-windows' },
      ...
      editing && el('span', { className: 'dm-window-edit' },
        ...
        el('button', { type: 'button', className: 'dm-icon-btn', 'aria-label': t('cancel'), onClick: () => setWinEdit(null) }, '\u00d7')))
  }
