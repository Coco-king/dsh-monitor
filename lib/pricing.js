...
/** 峰谷时代之前的官方基础价(人民币 / 1M tokens),按汇率 7.2 由美元基础价折算。 */
export const CNY_LEGACY_BASE_PRICES = {
  'deepseek-v4-flash': { cacheHit: 0.02016, cacheMiss: 1.008, output: 2.016 },
  'deepseek-v4-pro': { cacheHit: 0.0261, cacheMiss: 3.132, output: 6.264 },
}
...
/** 内置默认人民币价格表(与官方中文页当前数字一致;基础档 = 空闲档,legacyBase 按汇率折算)。 */
export const DEFAULT_PRICE_TABLE_CNY = {
  models: {
    'deepseek-v4-flash': {
      ...
      legacyBase: { ...CNY_LEGACY_BASE_PRICES['deepseek-v4-flash'] },
    },
    'deepseek-v4-pro': {
      cacheHit: 0.15,
      cacheMiss: 4.5,
      ...
      legacyBase: { ...CNY_LEGACY_BASE_PRICES['deepseek-v4-pro'] },
    },
  },
...
/** 评级目标的语言/内容差异:指标、模型表头、档位标签、历史基础价表。 */
const TARGET_PROFILE = {
  usd: {
    ...
    legacyPrices: LEGACY_BASE_PRICES,
  },
  cny: {
    metricOf: text => {
    ...
    legacyPrices: CNY_LEGACY_BASE_PRICES,
  },
}
...
export function parsePricingHtml(html, target = 'usd') {
  ...
  for (let k = 0; k < modelIds.length; k += 1) {
    ...
    const legacy = profile.legacyPrices[id]
