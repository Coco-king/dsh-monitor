...
/** 空桶(与账本行字段一一对应;requests = 调用次数,费用双币:USD/CNY)。 */
export function zeroBuckets() {
  return { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, requests: 0, costUsd: 0, costCny: 0 }
}
...
export function bucketsOf(usage) {
  return {
    ...
    requests: 1,
    costUsd: 0, // 由调用方按事件时刻 USD 价格表计费后回填
    costCny: 0, // 由调用方按事件时刻 CNY 价格表计费后回填
  }
}
 ...
 * @param options - { bill: ({buckets, atMs, provider, model}) => {costUsd, costCny} }
 *   计费函数。由调用方注入双币价格表与峰谷配置(不在此处依赖定价,便于纯函数单测);
 *   缺省时两币费用均为 0。
 */
export function applyUsageDelta(state, events, options = {}) {
  const bill = typeof options.bill === 'function'
    ? options.bill
    : () => ({ costUsd: 0, costCny: 0 })
  ...
  for (const event of events ?? []) {
    ...
    const priced = bill({ buckets, atMs, provider: route.provider, model: route.model })
    buckets.costUsd = num(priced?.costUsd)
    buckets.costCny = num(priced?.costCny)
 ...
 * @returns [{ provider, model, input, output, cacheRead, cacheWrite, requests, costUsd, costCny, day }]
 *   全零路由行会被丢弃。
 */
