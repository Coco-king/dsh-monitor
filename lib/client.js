...
function parseUsageSummary(v, path) {
  ...
  return { totals, byDay, models, sessions };
}
...
var require_chart = __commonJS({
  "lib/client-src/chart.js"(exports2, module2) {
    var { createElement: el } = require("react");
    var { Tooltip } = require("@deepseek-ai/dsh-client-ui-primitives");
    ...
    module2.exports = { BarChart, dayKey, shortLabel, tokensOf };
  }
});
...
var require_dashboard = __commonJS({
  "lib/client-src/dashboard.js"(exports2, module2) {
    ...
    var { BarChart, dayKey, shortLabel, tokensOf } = require_chart();
    ...
    function heatCells(byDay, windowDays = 91) {
      ...
      for (let back = windowDays - 1; back >= 0; back -= 1) {
        const at = new Date(today.getTime() - back * 864e5);
    ...
    function fmtTokens(n) {
      const v = Number(n ?? 0);
      if (v < 1e3) return String(Math.round(v));
      if (v < 1e6) return String(Math.round(v / 1e3 * 10) / 10) + "K";
      return String(Math.round(v / 1e6 * 10) / 10) + "M";
    }
    function Dashboard2({ wide, useMonitor, api, t, locale }) {
      ...
      const symbol = activeCurrency2(locale ?? config?.locale) === "cny" ? "\xA5" : "$";
      const fmtMoney = (v) => symbol + Number(v ?? 0).toFixed(decimals).replace(/\.?0+$/, "");
      const moneyOf = (row) => activeCurrency2(locale ?? config?.locale) === "cny" ? row?.costCny ?? 0 : row?.costUsd ?? 0;
      const [open, setOpen] = useState(false);
      ...
      useEffect(() => {
        ...
        const loadBadge = () => {
          api.getUsage({ range: RANGES[0].range() }).then(
            (v) => {
              if (alive) setTodayTotal(v.totals);
            },
            () => {
            }
          );
      ...
      useEffect(() => {
        ...
        Promise.all([
          ...
          api.getUsage({ range: RANGES[0].range() }),
          // 今日
          api.getUsage({ range: RANGES[1].range() }),
          // 本月
          api.getUsage({ range: RANGES[2].range() }),
          // 全部
          api.getUsage({ range: { start: daysAgoKey(91) } })
          // 热力带
        ]).then(
          ([summary, today, month, all, heat2]) => {
            if (!alive) return;
            setData(summary);
            setCards({ today: today.totals, month: month.totals, all: all.totals });
            if (todayTotal === null) setTodayTotal(today.totals);
            setHeatDays(heat2.byDay);
            setError(null);
          },
          (err) => {
          ...
          el(
            ...
            data !== null && el(
              ...
              el(
                ...
                el(BarChart, {
                  ...
                  tipOf: (row) => `${row.date} \xB7 ${fmtTokens(tokensOf(row))} tokens \xB7 ${fmtMoney(moneyOf(row))}`
                })
              ),
              // 模型表
              el(
                ...
                data.models.length === 0 ? el("div", { className: "dm-dash-note" }, t("noUsageSummary")) : el(
                  "table",
                  ...
                  el("tbody", null, data.models.map((m, i) => el(
                    "tr",
              ...
              el(
                ...
                data.sessions.length === 0 ? el("div", { className: "dm-dash-note" }, t("noUsageSummary")) : el(
                  "div",
                  { className: "dm-dash-sessions" },
                  data.sessions.slice(0, 50).map((row) => el(
                    Tooltip,
                    { key: row.id, label: t("sessionCostTitle") + ": " + fmtMoney(moneyOf(row)) + " \xB7 " + t("sessionCalls") + " " + Number(row.calls ?? 0), delayMs: 400 },
                    el(
                      "div",
                      { className: "dm-dash-session", title: row.id },
            ...
            heat !== null && el(
              ...
              el(
                ...
                el(
                  "div",
                  { className: "dm-dash-heat" },
                  el(
                    "div",
                    { className: "dm-dash-heat-grid", style: { gridTemplateColumns: `repeat(${heatWeeks}, 12px)` } },
                    heat.cells.map((cell, i) => cell === null ? el("span", { key: "pad" + i, className: "dm-dash-heat-pad" }) : el("span", {
                      key: cell.key,
                      className: "dm-dash-heat-cell",
                      "data-l": heat.levelAt(cell.tokens),
