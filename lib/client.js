  ...
  "lib/client-src/i18n.js"() {
    MESSAGES = {
      zh: {
        ...
        usageSummaryIntro: "\u6309\u5929\u67E5\u770B DSH \u7684 token \u7528\u91CF\u4E0E\u8D39\u7528\uFF0C\u53EF\u6309\u63D0\u4F9B\u65B9 / \u6A21\u578B / \u65F6\u95F4\u8303\u56F4\u7B5B\u9009\uFF08\u6570\u636E\u6765\u81EA\u4F1A\u8BDD\u65E5\u5FD7\uFF0C\u968F\u626B\u63CF\u5237\u65B0\uFF09\u3002",
        todayCard: "\u4ECA\u65E5",
        monthCard: "\u672C\u6708\u7D2F\u8BA1",
        cardTokens: "Token {tokens}",
        cardCost: "\u8D39\u7528 {amount}",
      ...
      en: {
        ...
        usageSummaryIntro: "DSH token usage and cost by day, filterable by provider / model / time range (fed from session logs, refreshed by the sweeper).",
        todayCard: "Today",
        monthCard: "This month",
        cardTokens: "Token {tokens}",
        cardCost: "Cost {amount}",
        ...
        sessionCalls: "Calls",
        sessionCost: "Cost",
        noUsageSummary: "No usage data for this filter.",
...
var require_panel = __commonJS({
  "lib/client-src/panel.js"(exports2, module2) {
    var { createElement: el2, Fragment, useState, useEffect, useCallback, useRef } = require("react");
    var { Tooltip: Tooltip2 } = require("@deepseek-ai/dsh-client-ui-primitives");
    ...
    function GaugeIcon({ size = 16, className }) {
      return el2(
        ...
        el2("path", { d: "M1.5 10.4A6.5 6.5 0 1 1 14.5 10.4", stroke: "currentColor", strokeWidth: "1.7", strokeLinecap: "round" }),
        el2("path", { d: "M8 10.4V4.2", stroke: "currentColor", strokeWidth: "1.7", strokeLinecap: "round" }),
        el2("circle", { cx: "8", cy: "10.4", r: "1.4", fill: "currentColor" })
      );
    }
    ...
    function UsageItemRow({ item, t }) {
      ...
      rows.push(el2(
        ...
        el2("div", { className: "dm-label", title: item.label }, item.label),
        pct !== null ? el2(
          Fragment,
          null,
          el2("div", { className: "dm-bar" }, el2("div", { className: "dm-fill" + fillClass, style: { width: pct + "%" } })),
          el2("div", { className: "dm-num" }, valueText)
        ) : el2("div", { className: "dm-num" }, valueText)
      ));
      if (showMax) {
        rows.push(el2("div", { key: "max", className: "dm-reset" }, item.label + ": " + formatPlain2(item.value, 2) + " / " + formatPlain2(item.max, 2)));
      }
      if (item.resetsAt) {
        rows.push(el2("div", { key: "reset", className: "dm-reset" }, t("resetsAt", { time: new Date(item.resetsAt).toLocaleString() })));
      }
      return el2("div", null, rows);
    }
    function UsagePanel(props) {
      ...
      let body;
      if (loading) {
        body = el2("div", { className: "dm-msg off" }, t("loading"));
      } else if (!configured && !isBuiltinDeepseek) {
        body = el2("div", { className: "dm-empty" }, t("notConfiguredHint"));
      } else if (error) {
        body = el2("div", { className: "dm-msg err" }, error);
      } else if (usage !== null && (usage.status === "error" || usage.status === "off" && usage.message.length > 0)) {
        body = el2("div", { className: "dm-msg " + (usage.status === "error" ? "err" : "off") }, usage.message);
      } else if (usage !== null && usage.items.length === 0) {
        body = el2("div", { className: "dm-empty" }, t("noUsageItems"));
      } else {
        body = usage === null ? el2("div", { className: "dm-msg off" }, t("loading")) : el2("div", { className: "dm-items" }, usage.items.map((item) => el2(UsageItemRow, { key: item.key, item, t })));
      }
      return el2(
        "div",
        ...
        el2(
          ...
          el2("div", { className: "dm-panel-title", title: providerId || void 0 }, providerId || t("unknownProvider")),
          presetLabel !== null && el2("span", { className: "dm-preset" }, presetLabel),
          el2("button", { type: "button", className: "dm-icon-btn", "aria-label": t("refresh"), disabled: refreshing || loading, onClick: onRefresh }, RefreshIcon({ size: 14, spin: refreshing }))
        ),
        body,
        usage !== null && usage.fetchedAt > 0 && el2(
          "div",
          { className: "dm-panel-foot" },
          el2("span", null, t("updatedAt", { time: new Date(usage.fetchedAt).toLocaleTimeString() }))
        )
      );
    }
    ...
    function SessionCost2(props) {
      ...
      return el2(
        ...
        el2("div", { className: "dm-chip" }, t("cost", { amount: money(cost) }))
      );
    }
...
function BarChart({ data, height = 140, valueOf, labelOf, tipOf, t }) {
  ...
  const bars = rows.map((row, i) => {
    ...
    return el(
      ...
      el(
        Tooltip,
        { label: tipOf(row), side: "top", delayMs: 150 },
...
var init_chart = __esm({
  "lib/client-src/chart.js"() {
    ({ createElement: el } = require("react"));
    ({ Tooltip } = require("@deepseek-ai/dsh-client-ui-primitives"));
  }
});
...
var require_settings = __commonJS({
  "lib/client-src/settings.js"(exports2, module2) {
    var { createElement: el2, useState, useEffect } = require("react");
    ...
    function FieldHint({ text }) {
      return el2(
        ...
        el2("span", { className: "dm-hint", role: "img", "aria-label": text, tabIndex: 0 }, "?")
      );
    }
    ...
    function ProviderForm(props) {
      ...
      const field = (label, control, hint) => el2(
        "div",
        { className: "dm-field" },
        el2(
          "span",
          { className: "dm-field-caption" },
          el2("label", null, label),
          hint !== void 0 && el2(FieldHint, { text: hint })
        ),
        control
      );
      ...
      return el2(
        ...
        el2(
          ...
          field(t("providerId"), el2(
            ...
            el2("option", { value: "", disabled: true }, t("providerIdDatalist")),
            providerOptions.map((o) => el2("option", { key: o.value, value: o.value }, o.label))
          )),
          field(t("refreshMinutes"), el2("input", {
            className: "dm-input",
            type: "number",
            min: "1",
            max: "1440",
        ...
        field(t("presetChoose"), el2(
          "select",
          { className: "dm-input", value: preset, onChange: (e) => setPreset(e.target.value) },
          el2("option", { value: "deepseek" }, t("presetOptDeepseek")),
          el2("option", { value: "opencode" }, t("presetOptOpencode")),
          el2("option", { value: "custom" }, t("presetOptCustom"))
        )),
        el2(
          "label",
          { className: "dm-switch", style: { width: "fit-content" } },
          el2("input", { type: "checkbox", checked: enabled, onChange: (e) => setEnabled(e.target.checked) }),
          t("enabled")
        ),
        preset === "deepseek" && el2("p", { className: "dm-note" }, t("deepseekHint")),
        preset === "opencode" && el2(
          "div",
          { className: "dm-field" },
          field(t("apiKey"), el2("input", {
            className: "dm-input",
            type: "password",
            value: apiKey,
          ...
          el2("p", { className: "dm-note" }, t("opencodeHint"))
        ),
        preset === "custom" && el2(
          "div",
          { className: "dm-field" },
          el2("p", { className: "dm-note dm-custom-intro" }, t("customIntro")),
          field(t("customUrl"), el2("input", { className: "dm-input", type: "text", value: url, onChange: (e) => setUrl(e.target.value) }), t("customUrlHint")),
          field(t("customHeaders"), el2(
            "div",
            { className: "dm-header-list" },
            headerPairs.map((pair, i) => el2(
              "div",
              { key: i, className: "dm-header-row" },
              el2("input", { className: "dm-input", "aria-label": t("headerKey"), placeholder: "Authorization", value: pair.key, onChange: (e) => updateHeader(i, "key", e.target.value) }),
              el2("input", { className: "dm-input", "aria-label": t("headerValue"), placeholder: "Bearer {apiKey}", value: pair.value, onChange: (e) => updateHeader(i, "value", e.target.value) }),
              el2("button", {
                type: "button",
            ...
            el2("button", { type: "button", className: "dm-btn ghost small", style: { justifySelf: "start" }, onClick: () => setHeaderPairs((list) => [...list, { key: "", value: "" }]) }, t("addHeader")),
            el2("p", { className: "dm-note" }, t("customHeadersNote"))
          ), t("customHeadersHint")),
          el2(
            "div",
            { className: "dm-field" },
            el2("label", null, t("customItems")),
            el2(
              "div",
              ...
              items.map((it, i) => el2(
                ...
                el2(
                  ...
                  el2("span", { className: "dm-item-title" }, t("itemField") + " " + String(i + 1)),
                  el2("button", {
                    type: "button",
                    className: "dm-icon-btn danger",
                    "aria-label": t("remove"),
                ...
                el2(
                  "div",
                  { className: "dm-grid2" },
                  field(tItem("key"), el2("input", { className: "dm-input", type: "text", placeholder: goExample(i).key, value: it.key, onChange: (e) => updateItem(i, "key", e.target.value) }), t("itemKeyHint")),
                  field(tItem("label"), el2("input", { className: "dm-input", type: "text", placeholder: t(goExample(i).labelKey), value: it.label, onChange: (e) => updateItem(i, "label", e.target.value) }), t("itemLabelHint"))
                ),
                el2(
                  "div",
                  { className: "dm-grid2" },
                  field(tItem("kind"), el2(
                    "select",
                    { className: "dm-input", value: it.kind, onChange: (e) => updateItem(i, "kind", e.target.value) },
                    el2("option", { value: "percent" }, t("kindPercent")),
                    el2("option", { value: "number" }, t("kindNumber")),
                    el2("option", { value: "money" }, t("kindMoney")),
                    el2("option", { value: "text" }, t("kindText"))
                  ), t("itemKindHint")),
                  field(tItem("path"), el2("input", { className: "dm-input", type: "text", placeholder: "usage." + goExample(i).key + ".percent", value: it.path, onChange: (e) => updateItem(i, "path", e.target.value) }), t("itemPathHint"))
                ),
                el2(
                  "div",
                  { className: "dm-grid2" },
                  field(tItem("maxPath"), el2("input", { className: "dm-input", type: "text", placeholder: "1000000", value: String(it.maxPath ?? ""), onChange: (e) => updateItem(i, "maxPath", e.target.value) }), t("itemMaxPathHint")),
                  field(tItem("resetsAtPath"), el2("input", { className: "dm-input", type: "text", placeholder: "usage." + goExample(i).key + ".resetsAt", value: String(it.resetsAtPath ?? ""), onChange: (e) => updateItem(i, "resetsAtPath", e.target.value) }), t("itemResetsAtHint"))
                )
              ))
            ),
            el2("button", { type: "button", className: "dm-btn ghost small", style: { alignSelf: "flex-start" }, onClick: () => setItems((list) => [...list, { key: "", label: "", kind: "percent", path: "", maxPath: "", resetsAtPath: "" }]) }, t("addItem"))
          ),
          el2(
            "details",
            { className: "dm-custom-explain" },
            el2("summary", null, t("customItemsExplainTitle")),
            el2(
              "div",
              { className: "dm-custom-explain-body" },
              el2("p", { className: "dm-note" }, t("customItemsExplain")),
              el2(
                "div",
                { className: "dm-row-actions" },
                el2("button", { type: "button", className: "dm-btn ghost small", onClick: () => setShowSample(true) }, t("viewSample"))
              )
            )
          )
        ),
        el2(
          ...
          el2("button", { type: "button", className: "dm-btn ghost", onClick: onCancel }, t("cancel")),
          el2("button", { type: "button", className: "dm-btn", onClick: submit, disabled: provider.trim().length === 0 || preset === "custom" && url.trim().length === 0 }, t("save"))
        ),
        showSample && createPortal(
          el2(
            ...
            el2(
              ...
              el2("img", { src: SAMPLE_IMG, alt: t("viewSample"), className: "dm-img" }),
              el2(
                "div",
                { className: "dm-img-actions" },
                el2("button", { type: "button", className: "dm-btn ghost small", onClick: () => setShowSample(false) }, t("cancel"))
              )
            )
          ),
          document.body
    ...
    function PricesSection(props) {
      ...
      const numberInput = (modelId, field, label) => el2("input", {
      ...
      const tierNumberInput = (modelId, tierKey, field, label) => el2("input", {
        className: "dm-input dm-num",
        type: "number",
        step: "0.000001",
        min: "0",
      ...
      const ApplyDownIcon = ({ size = 14 } = {}) => el2(
        "svg",
        { width: size, height: size, viewBox: "0 0 16 16", fill: "none", xmlns: "http://www.w3.org/2000/svg", "aria-hidden": "true" },
        el2("path", { d: "M8 2.5V10.5M4.5 7l3.5 3.5L11.5 7", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round", strokeLinejoin: "round" }),
        el2("path", { d: "M3 12.5h10v1H3z", fill: "currentColor" })
      );
      ...
      const caption = () => el2(
        ...
        el2("span", null, t("modelName")),
        el2("span", null, t("cacheHit")),
        el2("span", null, t("cacheMiss")),
        el2("span", null, t("output")),
        el2("span", null, t("actions"))
      );
      const tierWindowsBlock = (modelId, tier) => {
        ...
        return el2(
          ...
          el2("span", { className: "dm-window-label" }, t("windowsLabel")),
          wins.map(tag),
          !editing && el2("button", {
            type: "button",
          ...
          editing && el2(
            ...
            el2("input", { className: "dm-input dm-num", type: "time", step: "3600", value: winStart, "aria-label": t("windowStart"), onChange: (e) => setWinStart(e.target.value) }),
            "\u2013",
            el2("input", { className: "dm-input dm-num", type: "time", step: "3600", value: winEnd, "aria-label": t("windowEnd"), onChange: (e) => setWinEnd(e.target.value) }),
            el2("button", { type: "button", className: "dm-btn ghost small", onClick: addWindow }, t("save")),
            el2("button", { type: "button", className: "dm-icon-btn", "aria-label": t("cancel"), onClick: () => {
              setWinEdit(null);
              setWinError(null);
            } }, "\xD7")
          ),
          editing && winError !== null && el2("span", { className: "dm-window-error" }, winError)
        );
      };
      return el2(
        "div",
        ...
        el2(
          ...
          el2("h2", { className: "dm-h" }, t("pricesTitle")),
          el2(
            "div",
            { className: "dm-row-actions" },
            el2("button", { type: "button", className: "dm-btn ghost", onClick: sync, disabled: syncing || busy }, t("syncFromDocs")),
            el2("button", { type: "button", className: "dm-btn", onClick: save, disabled: busy }, t("save"))
          )
        ),
        el2("p", { className: "dm-intro" }, t("peakNotice")),
        el2("p", { className: "dm-note" }, t("lastSync", { time: now, source })),
        notice !== null && el2("p", { className: "dm-notice " + (notice.kind === "err" ? "err" : "ok") }, notice.text),
        el2(
          "div",
            ...
            return el2(
              ...
              el2(
                ...
                el2(
                  ...
                  modelId,
                  el2("span", { className: "dm-price-base" }, t("basePrice"))
                ),
                ...
                el2(
                  ...
                  el2(
                    ...
                    el2("input", { type: "checkbox", checked: open, onChange: () => toggleTier(modelId) }),
                    t("tiersToggle")
                  ),
                  el2("button", {
                    type: "button",
              ...
              open && el2(
                ...
                el2(
                  ...
                  el2("span", { className: "dm-tier-name" }, t("offPeak")),
                ...
                el2(
                  "div",
                  { className: "dm-tier-row" },
                  el2("span", { className: "dm-tier-name" }, t("peak")),
          ...
          el2(
            ...
            el2(
              "div",
              { className: "dm-price-fields" },
              el2("span", { className: "dm-price-name" }, t("defaultModel")),
        ...
        el2(
          ...
          addingModel && el2(
            ...
            el2(
              ...
              el2("span", { className: "dm-editor-title" }, t("addModelTitle"))
            ),
            el2(
              "div",
              { className: "dm-field" },
              el2("label", null, t("modelSelectHint")),
              el2(
                "select",
                { className: "dm-input", value: pickedModel, onChange: (e) => setPickedModel(e.target.value) },
                el2("option", { value: "", disabled: true }, "\u2026"),
                // 排除已设置过定价的模型(已在草稿中的)。
                modelGroups.map((group) => {
                  ...
                  return el2(
                    ...
                    pending.map((m) => el2("option", { key: group.id + ":" + m.id, value: m.id }, m.name && m.name !== m.id ? `${m.name} (${m.id})` : m.id))
                  );
                })
              ),
              // 所有模型都已定价:明确提示,避免下拉空白令人困惑(目录为空时不算)。
              (catalog?.models?.length ?? 0) > 0 && modelGroups.every((g) => g.models.every((m) => draft.models[m.id] !== void 0)) && el2("p", { className: "dm-note" }, t("noModelsLeft"))
            ),
            el2(
              "div",
              { className: "dm-grid3" },
              el2(
                "div",
                { className: "dm-field" },
                el2("label", null, t("cacheHit")),
                el2("input", { className: "dm-input", type: "number", step: "0.000001", min: "0", value: newHit, onChange: (e) => setNewHit(e.target.value) })
              ),
              el2(
                "div",
                { className: "dm-field" },
                el2("label", null, t("cacheMiss")),
                el2("input", { className: "dm-input", type: "number", step: "0.000001", min: "0", value: newMiss, onChange: (e) => setNewMiss(e.target.value) })
              ),
              el2(
                "div",
                { className: "dm-field" },
                el2("label", null, t("output")),
                el2("input", { className: "dm-input", type: "number", step: "0.000001", min: "0", value: newOutput, onChange: (e) => setNewOutput(e.target.value) })
              )
            ),
            el2(
              "div",
              ...
              el2("button", { type: "button", className: "dm-btn", onClick: addPicked, disabled: String(pickedModel ?? "").trim().length === 0 }, t("addModel"))
            )
          ),
          !addingModel && el2("button", { type: "button", className: "dm-btn add", onClick: () => setAddingModel(true) }, t("addModel"))
        )
      );
    }
    var { BarChart: BarChart2, dayKey: dayKey2, monthStartKey: monthStartKey2, shortLabel: shortLabel2, tokensOf: tokensOf2 } = (init_chart(), __toCommonJS(chart_exports));
    var RANGE_PRESETS = ["today", "7d", "30d", "month", "custom"];
    var RANGE_KEY = { today: "rangeToday", "7d": "range7d", "30d": "range30d", month: "rangeMonth", custom: "rangeCustom" };
    var CARD_RANGE = { today: { start: dayKey2(0) }, month: { start: monthStartKey2() } };
    function UsageSummarySection(props) {
      const { api, t, locale, config, catalog } = props;
      const currencyFor = () => {
        const active = (init_format(), __toCommonJS(format_exports)).activeCurrency;
        return active(locale ?? config?.locale);
      };
      const symbol = currencyFor() === "cny" ? "\xA5" : "$";
      ...
      const fmtTokens = (v) => {
        ...
        if (n < 1e6) return String(Math.round(n / 1e3 * 10) / 10) + "K";
        return String(Math.round(n / 1e6 * 10) / 10) + "M";
      };
      const [preset, setPreset] = useState("7d");
      const [customFrom, setCustomFrom] = useState(dayKey2(-6));
      const [customTo, setCustomTo] = useState(dayKey2(0));
      ...
      const [error, setError] = useState(null);
      const [loading, setLoading] = useState(false);
      const providerModels = catalog?.models?.filter((m) => m.provider === provider).map((m) => ({ id: m.id, name: m.name || m.id })) ?? [];
      const providerOptions = catalog?.providers ?? [];
      const rangeOf = () => {
        if (preset === "custom") return { start: customFrom, end: customTo };
        if (preset === "today") return { start: dayKey2(0) };
        if (preset === "7d") return { start: dayKey2(-6) };
        if (preset === "30d") return { start: dayKey2(-29) };
        return { start: monthStartKey2() };
      };
      const load = () => {
        setLoading(true);
      ...
      const card = (label, summary) => el2(
        "div",
        { key: label, className: "dm-summary-card" },
        el2("div", { className: "dm-summary-card-label" }, label),
        el2("div", { className: "dm-summary-card-val" }, fmtTokens(summary?.totals ? tokensOf2(summary.totals) : 0)),
        el2(
          "div",
          { className: "dm-summary-card-sub" },
          t("cardCost", { amount: fmtMoney(summary?.totals?.cost ?? 0) }) + " \xB7 " + t("cardCalls", { calls: Number(summary?.totals?.calls ?? 0) })
        )
      );
      return el2(
        "div",
        ...
        el2(
          "div",
          { className: "dm-toolbar" },
          el2("h2", { className: "dm-h" }, t("usageSummaryTitle"))
        ),
        el2("p", { className: "dm-intro" }, t("usageSummaryIntro")),
        el2(
          "div",
          { className: "dm-row dm-filter-row" },
          RANGE_PRESETS.map((key) => el2("button", {
            key,
            type: "button",
          ...
          preset === "custom" && el2(
            "div",
            { className: "dm-row dm-filter-range" },
            el2("input", { className: "dm-input", type: "date", value: customFrom, "aria-label": t("rangeStart"), onChange: (e) => setCustomFrom(e.target.value) }),
            "\u2013",
            el2("input", { className: "dm-input", type: "date", value: customTo, "aria-label": t("rangeEnd"), onChange: (e) => setCustomTo(e.target.value) })
          )
        ),
        el2(
          "div",
          { className: "dm-row dm-filter-row" },
          el2("label", { className: "dm-filter-label" }, t("providerFilter")),
          el2(
            "select",
            { className: "dm-input", value: provider, onChange: (e) => {
              setProvider(e.target.value);
              setModel("");
            } },
            el2("option", { value: "" }, t("providerAll")),
            providerOptions.map((p) => el2("option", { key: p.id, value: p.id }, p.name || p.id))
          ),
          el2("label", { className: "dm-filter-label" }, t("modelFilter")),
          el2(
            "select",
            { className: "dm-input", value: model, onChange: (e) => setModel(e.target.value), disabled: provider === "" },
            el2("option", { value: "" }, t("modelAll")),
            providerModels.map((m) => el2("option", { key: m.id, value: m.id }, m.name || m.id))
          )
        ),
        el2(
          "div",
          { className: "dm-summary-cards" },
          cards === null ? el2("div", { className: "dm-empty" }, t("loading")) : el2(
            "div",
            { className: "dm-summary-cards" },
        ...
        loading && data === null && el2("div", { className: "dm-empty" }, t("loading")),
        error !== null && el2("div", { className: "dm-empty" }, error),
        data !== null && el2(
          "div",
          { className: "dm-subsection" },
          el2(
            "div",
            { className: "dm-toolbar" },
            el2("h2", { className: "dm-h" }, t("chartByDay"))
          ),
          el2(BarChart2, {
            data: data.byDay,
            t,
            height: 150,
            valueOf: (row) => tokensOf2(row),
            labelOf: (row) => shortLabel2(row.date),
            tipOf: (row) => `${row.date} \xB7 ${t("cardTokens", { tokens: fmtTokens(tokensOf2(row)) })} \xB7 ${t("cardCost", { amount: fmtMoney(row.cost) })}`
          }),
          el2(
            "div",
            { className: "dm-toolbar" },
            el2("h2", { className: "dm-h" }, t("sessionListTitle"))
          ),
          data.sessions.length === 0 ? el2("div", { className: "dm-empty" }, t("noUsageSummary")) : el2(
            "div",
            { className: "dm-table-wrap" },
            el2(
              "table",
              { className: "dm-table" },
              el2("thead", null, el2(
                "tr",
                null,
                el2("th", null, t("sessionId")),
                el2("th", null, t("sessionDate")),
                el2("th", { className: "num" }, t("sessionTokens")),
                el2("th", { className: "num" }, t("sessionCalls")),
                el2("th", { className: "num" }, t("sessionCost"))
              )),
              el2("tbody", null, data.sessions.slice(0, 100).map((row) => el2(
                "tr",
                { key: row.id },
                el2("td", { className: "id", title: row.id }, row.id.length > 20 ? row.id.slice(0, 20) + "\u2026" : row.id),
                el2("td", null, row.date),
                el2("td", { className: "num" }, fmtTokens(tokensOf2(row))),
                el2("td", { className: "num" }, String(Number(row.calls ?? 0))),
                el2("td", { className: "num" }, fmtMoney(row.cost))
              )))
            )
          )
        )
    ...
    function SettingsSection2(props) {
      ...
      if (config === null || config === void 0) {
        return el2(
          ...
          el2("div", { className: "dm-empty" }, status === "error" ? snap?.error ?? "" : t("loading"))
        );
      }
      return el2(
        "div",
        { className: "dm-section" },
        el2(PricesSection, { config, api, t, catalog, locale: locale ?? config?.locale }),
        el2(UsageSummarySection, { config, api, t, catalog })
      );
    }
...
var require_binding = __commonJS({
  "lib/client-src/binding.js"(exports2, module2) {
    var { createElement: el2, useState } = require("react");
      ...
      return el2(
        ...
        el2(
          ...
          el2(
            ...
            el2("span", { className: "dm-bind-title" }, t("bindingTitle")),
            el2("span", { className: "dm-bind-sub" }, title),
            el2("button", { type: "button", className: "dm-icon-btn danger", style: { marginLeft: "auto" }, "aria-label": t("cancel"), onClick: onClose }, "\xD7")
          ),
          el2("p", { className: "dm-note" }, t("bindingDesc")),
          notice !== null && el2("p", { className: "dm-notice " + (notice.kind === "err" ? "err" : "ok") }, notice.text),
          el2(ProviderForm, {
          ...
          existing !== void 0 && el2(
            ...
            el2("button", { type: "button", className: "dm-btn danger small", onClick: remove, disabled: busy }, t("bindRemove"))
          )
        )
      );
    }
