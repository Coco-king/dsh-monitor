  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        addProvider: '添加提供方',
        providerId: '提供方 ID',
        providerIdPlaceholder: '从 设置→模型 已配置的提供方中选择,也可手输',
        providerIdDatalist: '选择提供方…',
        modelSelectHint: '从 设置→模型 中选择要添加价格的模型(按提供方分组)',
        addModelManual: '或手输模型 ID(不在目录时)',
      ...
      en: {
        ...
        providerIdPlaceholder: 'Pick a provider configured in Settings → Models, or type one',
    ...
    const { createElement: el, Fragment, useState, useEffect, useCallback, useRef, useId } = React
    ...
    function ProviderForm(props) {
      ...
      const providerListId = 'dm-providers-' + (useId().replace(/:/g, '') || 'x')
      ...
      return el('div', { className: 'dm-provider-card' },
        el('div', { className: 'dm-grid2' },
          field(t('providerId'), el('div', null,
            el('input', {
              className: 'dm-input', type: 'text', list: providerListId,
              value: provider,
              placeholder: t('providerIdPlaceholder'),
              disabled: initial !== null,
              onChange: e => setProvider(e.target.value),
            }),
            el('datalist', { id: providerListId },
              providerOptions.map(o => el('option', { key: o.value, value: o.value }, o.label))),
            el('p', { className: 'dm-note' }, t('providerIdDatalist')))),
        ...
        providerIds.map(providerId => {
          ...
          if (editing === providerId) {
            return el(ProviderForm, {
              key: providerId, initial: { ...provider, provider: providerId }, onSave: save, onCancel: () => setEditing(null), t, options: providerCandidates,
            })
          }
    ...
    function PricesSection(props) {
      const { config, api, t, catalog } = props
      const modelListId = 'dm-models-' + (useId().replace(/:/g, '') || 'x')
      ...
      return el('div', null,
        ...
        el('div', { className: 'dm-row-actions', style: { marginTop: 8 } },
          el('div', { className: 'dm-field', style: { minWidth: 280 } },
            el('label', null, t('modelSelectHint')),
            el('div', { className: 'dm-row-actions' },
              el('select', { className: 'dm-input', style: { maxWidth: 320, width: '100%' }, value: pickedModel, onChange: e => setPickedModel(e.target.value) },
              ...
              el('button', { type: 'button', className: 'dm-btn ghost small', onClick: addPicked, disabled: pickedModel.length === 0 }, t('addModel')))),
          el('button', { type: 'button', className: 'dm-btn', onClick: save, disabled: busy }, t('save'))),
        el('div', { className: 'dm-row-actions', style: { marginTop: 8 } },
          el('div', { className: 'dm-field', style: { minWidth: 220 } },
            el('label', null, t('addModelManual')),
            el('input', { className: 'dm-input', type: 'text', list: modelListId, placeholder: t('newModelPlaceholder'), value: newModel, onChange: e => setNewModel(e.target.value), style: { maxWidth: 320 } }),
            el('datalist', { id: modelListId },
              modelGroups.flatMap(group => group.models.map(m => ({ value: m.id, label: `${(m.name && m.name !== m.id ? m.name : m.id)} · ${group.name}` }))).map(o => el('option', { key: o.value, value: o.value }, o.label)))),
    ...
    /** 构建标记:每次客户端改动后更新(用于确认热更是否生效;服务端需重启才能吐出新 bundle)。 */
    const BUILD_TAG = '0692f9d'
