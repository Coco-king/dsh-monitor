...
function tierFor(entry, atMs, peak) {
  ...
  const peakWins = Array.isArray(own?.peak) && own.peak.length > 0 ? own.peak : Array.isArray(peak.windows) && peak.windows.length > 0 ? peak.windows : [];
  ...
  "lib/client-src/settings.js"(exports2, module2) {
    ...
    function PricesSection(props) {
      ...
      const addWindow = () => {
        ...
        const wins = [...windowsOf(winEdit.modelId, winEdit.tier), { start: localHourToUtc2(from), end: localHourToUtc2(to) }];
      ...
      const tierWindowsBlock = (modelId, tier) => {
        ...
        return el(
          ...
          editing && el(
            ...
            el("button", { type: "button", className: "dm-icon-btn", "aria-label": t("cancel"), onClick: () => setWinEdit(null) }, "\xD7")
          )
        );
      };
      return el(
