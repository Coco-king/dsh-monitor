...
var init_styles = __esm({
  "lib/client-src/styles.js"() {
    css = [
      ...
      // 价格列固定等宽且有列序:模型名 | 操作 | 命中 | 未命中 | 输出。
      // 操作列固定 150px 置于输入框左侧(表头「操作」与各行的 1.6fr 名称列占同一空档,
      // 代价是峰谷「高峰」行该格留白)——auto 会让各格里表头与开关宽度不同,把名称列
      // 二端的价格表整体顶偏,因此必须固定;输入框 68px 收窄,足够容纳四位小数。
      ".dm-price-caption{display:grid;grid-template-columns:minmax(0,1.6fr) 150px repeat(3,68px);gap:8px;padding:0 9px;font-size:12px;line-height:18px;font-weight:500;color:var(--dsw-alias-label-secondary)}",
      ...
      ".dm-price-fields{display:grid;grid-template-columns:minmax(0,1.6fr) 150px repeat(3,68px);gap:8px;align-items:center}",
      ...
      ".dm-tier-row{display:grid;grid-template-columns:minmax(0,1.6fr) 150px repeat(3,68px);gap:8px;align-items:center}",
