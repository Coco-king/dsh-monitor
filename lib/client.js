...
var init_styles = __esm({
  "lib/client-src/styles.js"() {
    css = [
      ...
      // 开关统一为滑块(custom checkbox):label 的 ::before 轨道 + ::after 滚珠,
      // 原生 checkbox 隐藏但保留在 label 内(点击仍走 label→input 切换);:has 选中态贴色。
      ".dm-switch{position:relative;cursor:pointer;user-select:none;display:inline-flex;align-items:center;padding-left:40px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary)}",
      ".dm-switch input[type=checkbox]{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}",
      '.dm-switch::before{content:"";position:absolute;top:50%;left:0;width:32px;height:18px;border-radius:999px;transform:translateY(-50%);background:var(--dsw-alias-fill-l2,rgba(128,128,128,.3));transition:background .15s}',
      '.dm-switch::after{content:"";position:absolute;top:50%;left:2px;width:14px;height:14px;border-radius:50%;transform:translateY(-50%);background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.25);transition:left .15s}',
      ".dm-switch:has(input:checked)::before{background:var(--dsw-alias-state-success-primary)}",
      ".dm-switch:has(input:checked)::after{left:16px}",
      // ── 设置页:自定义用量条目 ────────────────────────────────────────────────
