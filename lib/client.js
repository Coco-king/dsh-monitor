...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsSection(props) {
      ...
      console.log(`[dsh-monitor] section render: status=${JSON.stringify(status)} config=${config && typeof config === 'object' ? 'ok:' + Object.keys(config).length : 'NO'} at=${Date.now()}`)
    ...
    async function apply(ctx) {
      // ── 诊断:统计 apply 次数与 store 编号(排查设置页一直加载中) ──────────
      const applyN = (globalThis.__dshMonitorApplies = (globalThis.__dshMonitorApplies ?? 0) + 1)
      console.log(`[dsh-monitor] apply#${applyN} start`)
      ...
      const storeN = (globalThis.__dshMonitorStores = (globalThis.__dshMonitorStores ?? 0) + 1)
      const store = makeStore({ status: 'loading', error: null, config: null })
      store.__id = storeN
      store.__apply = applyN
      console.log(`[dsh-monitor] apply#${applyN} store#${storeN}`)
      ...
      const reload = async () => {
        ...
        try {
          ...
          console.log(`[dsh-monitor] apply#${applyN} store#${storeN} reload ok ${Date.now() - started}ms, keys=${config ? Object.keys(config).length : 0}`)
          store.set({ status: 'ready', error: null, config })
        } catch (error) {
          console.error(`[dsh-monitor] apply#${applyN} store#${storeN} reload FAIL ${Date.now() - started}ms —`, error?.message ?? String(error))
