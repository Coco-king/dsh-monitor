...
var init_styles = __esm({
  "lib/client-src/styles.js"() {
    css = [
      ...
      // 价格列固定等宽(px,不随字体/内容变化):基础价/空闲/高峰输入框完全同宽。
      // 「操作」列也固定 150px(auto 会随各格内容——表头只有「操作」、数据行是「峰谷开关+×」——
      // 宽窄不同,而 1.6fr 名称列会吸收这差异,把后面的价格列表头整体顶偏;固定后表头与
      // 各行的 1.6fr 名称列占同一宽度,命中/未命中/输出/操作逐列对齐)。
      ".dm-price-caption{display:grid;grid-template-columns:minmax(0,1.6fr) repeat(3,80px) 150px;gap:8px;padding:0 9px;font-size:12px;line-height:18px;font-weight:500;color:var(--dsw-alias-label-secondary)}",
      ...
      ".dm-price-fields{display:grid;grid-template-columns:minmax(0,1.6fr) repeat(3,80px) 150px;gap:8px;align-items:center}",
      ...
      ".dm-tier-row{display:grid;grid-template-columns:minmax(0,1.6fr) repeat(3,80px) 150px;gap:8px;align-items:center}",
