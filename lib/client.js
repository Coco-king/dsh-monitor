  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        modelSelectHint: '从 设置→模型 已配置的模型中选择,也可手输',
      ...
      en: {
        ...
        modelSelectHint: 'Pick a model configured in Settings → Models, or type one',
    ...
    function ProviderForm(props) {
      const { initial, onSave, onCancel, t, catalog } = props
      ...
      const providerOptions = delist((catalog?.providers ?? []).map(p => ({ value: p.id, label: p.name || p.id })))
    ...
    function ProvidersSection(props) {
      ...
      return el('div', null,
        ...
        providerIds.map(providerId => {
          ...
          if (editing === providerId) {
            return el(ProviderForm, {
              key: providerId, initial: { ...provider, provider: providerId }, onSave: save, onCancel: () => setEditing(null), t, catalog,
            })
          }
        ...
        adding && editing === 'new' && el(ProviderForm, { key: '__new', initial: null, onSave: save, onCancel: () => { setAdding(false); setEditing(null) }, t, catalog }))
    }
    ...
    function PricesSection(props) {
      ...
      const modelOptions = delist((catalog?.models ?? []).map(m => ({ value: m.id, label: (m.name || m.id) + (m.providerName ? ' · ' + m.providerName : '') })))
        ...
        el('div', { className: 'dm-row-actions', style: { marginTop: 8 } },
          el('div', { className: 'dm-field', style: { minWidth: 220 } },
            ...
            el('datalist', { id: modelListId },
              modelOptions.map(o => el('option', { key: o.value, value: o.value }, o.label)))),
