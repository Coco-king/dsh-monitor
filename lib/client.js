...
var init_styles = __esm({
  "lib/client-src/styles.js"() {
    css = [
      ...
      // 开关统一为滑块:label 内 隐藏 checkbox + .dm-switch-ui 轨道(::before 滚珠)。
      // 用 `input:checked + .dm-switch-ui` 兄弟选择器(比 :has 更利落),过渡 0.1s 快捷。
      ".dm-switch{position:relative;cursor:pointer;user-select:none;display:inline-flex;align-items:center;gap:8px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary)}",
      ".dm-switch input[type=checkbox]{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}",
      ".dm-switch-ui{position:relative;flex:none;width:32px;height:18px;border-radius:999px;background:var(--dsw-alias-fill-l2,rgba(128,128,128,.3));transition:background .1s}",
      '.dm-switch-ui::before{content:"";position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.25);transition:left .1s}',
      ".dm-switch input[type=checkbox]:checked + .dm-switch-ui{background:var(--dsw-alias-state-success-primary)}",
      ".dm-switch input[type=checkbox]:checked + .dm-switch-ui::before{left:16px}",
      // ── 设置页:自定义用量条目 ────────────────────────────────────────────────
      ...
      // 价格列固定等宽且有列序:模型名 | 开关槽 | 命中 | 未命中 | 输出 | 删除(×)。
      // 表头只显示 模型名/命中/未命中/输出(开关槽与删除列空占位,维持逐列对齐)。
      // 开关槽只放滑块本身(32px 轨)、右对齐贴住「命中」输入框——文字省给模型名,
      // 保证「模型名+基础价」不挤压;删除 × 在最后一列。
      ".dm-price-caption{display:grid;grid-template-columns:minmax(0,1.6fr) 44px repeat(3,68px) 28px;gap:8px;padding:0 9px;font-size:12px;line-height:18px;font-weight:500;color:var(--dsw-alias-label-secondary)}",
      ...
      ".dm-price-fields{display:grid;grid-template-columns:minmax(0,1.6fr) 44px repeat(3,68px) 28px;gap:8px;align-items:center}",
      // 名称列:模型名可收缩省略(text 子元素 flex:1 + ellipsis),「基础价」徽标 flex:none
      // 固定在末尾——否则长模型名把徽标顶到列右边缘外,被 overflow 裁掉半边。
      ".dm-price-name{display:flex;align-items:center;gap:6px;min-width:0;font-size:14px;line-height:22px;font-weight:500;color:var(--dsw-alias-label-primary);overflow:hidden}",
      ".dm-price-name-text{min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      ...
      ".dm-tier-row{display:grid;grid-template-columns:minmax(0,1.6fr) 44px repeat(3,68px) 28px;gap:8px;align-items:center}",
      ...
      // 峰谷「应用基础价」按钮与主行开关同槽(开关槽第二位),右对齐贴住「命中」列。
      ".dm-tier-row .dm-icon-btn{width:24px;height:24px;justify-self:end}",
      ...
      ".dm-window-tag.inherited{border-style:dashed;color:var(--dsw-alias-label-tertiary)}",
      ".dm-window-inherit-note{color:var(--dsw-alias-label-tertiary);font-size:11px}",
  ...
  "lib/client-src/i18n.js"() {
    MESSAGES = {
      zh: {
        ...
        windowsInherited: "\u5B98\u65B9\u9ED8\u8BA4",
        windowsInheritedHint: "\u672A\u70B9\u300C\u4ECE\u5B98\u65B9\u6587\u6863\u540C\u6B65\u300D\u65F6\u6CBF\u7528\u7684\u5168\u5C40\u5B98\u65B9\u9ED8\u8BA4\u9AD8\u5CF0\u65F6\u6BB5\uFF1B\u4E3A\u672C\u6A21\u578B\u6DFB\u52A0\u4EFB\u4E00\u81EA\u5B9A\u4E49\u65F6\u6BB5\u540E\u5373\u8F6C\u4E3A\u8BE5\u6A21\u578B\u81EA\u8BBE\u7A97\u53E3\u3002",
      ...
      en: {
        ...
        windowsInherited: "Official default",
        windowsInheritedHint: "The global official default peak windows, in effect until you sync from the docs. Adding a custom window for this model converts them into the model's own windows.",
...
var require_panel = __commonJS({
  "lib/client-src/panel.js"(exports2, module2) {
    ...
    function UsagePanel(props) {
      ...
      const isBuiltinOpencode = typeof providerId === "string" && /opencode/i.test(providerId);
      const preset = configured?.preset ?? (isBuiltinDeepseek ? "deepseek" : isBuiltinOpencode ? "opencode" : void 0);
      ...
      } else if (!configured && !isBuiltinDeepseek && !isBuiltinOpencode) {
    ...
    function UsageButton2(props) {
      const { sessionId, api, providerOf, useMonitor } = props;
      const storeSnap = useMonitor ? useMonitor((s) => s) : {};
      const config = storeSnap.config;
      const t = makeT3(storeSnap.locale ?? resolveLocale3(config?.locale));
...
var require_settings = __commonJS({
  "lib/client-src/settings.js"(exports2, module2) {
    ...
    function ProviderForm(props) {
      ...
      return el(
        ...
        el(
          ...
          el("span", { className: "dm-switch-ui" }),
          t("enabled")
        ),
    ...
    function PricesSection(props) {
      ...
      const entryHasOwnWindows = (entry) => {
        const windows = entry?.windows;
        return windows !== void 0 && windows !== null && (Array.isArray(windows.peak) && windows.peak.length > 0 || Array.isArray(windows.offPeak) && windows.offPeak.length > 0);
      };
      const effectiveWindows = (modelId, tier) => {
        if (entryHasOwnWindows(draft.models[modelId])) return windowsOf(modelId, tier);
        if (tier === "peak" && Array.isArray(config?.peakWindows)) return config.peakWindows;
        return [];
      };
      ...
      const setWindows = (modelId, tier, wins) => {
        setDraft((d) => {
          ...
          const windows = { ...entry.windows ?? {} };
          if (wins.length > 0 && !entryHasOwnWindows(entry) && Array.isArray(config?.peakWindows) && config.peakWindows.length > 0) {
            windows.peak = config.peakWindows.map((w) => ({ start: w.start, end: w.end }));
          }
      ...
      const addWindow = () => {
        ...
        if (effectiveWindows(winEdit.modelId, winEdit.tier).some((w) => windowsOverlap(w, candidate))) {
          setWinError(t("windowsOverlap"));
          return;
        }
        setWinError(null);
        const wins = [...effectiveWindows(winEdit.modelId, winEdit.tier), candidate];
      ...
      const CopyIntoIcon = ({ size = 14 } = {}) => el(
        "svg",
        { width: size, height: size, viewBox: "0 0 16 16", fill: "none", xmlns: "http://www.w3.org/2000/svg", "aria-hidden": "true" },
        el("path", { d: "M5.5 3h6A1.5 1.5 0 0 1 13 4.5v6a1.5 1.5 0 0 1-1.5 1.5h-6A1.5 1.5 0 0 1 4 10.5v-6A1.5 1.5 0 0 1 5.5 3Z", stroke: "currentColor", strokeWidth: "1.5", strokeLinejoin: "round" }),
        el("path", { d: "M3 8.5V4A1.5 1.5 0 0 1 4.5 2.5H9", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" })
      );
      ...
      const tierWindowsBlock = (modelId, tier) => {
        const wins = effectiveWindows(modelId, tier);
        const inherited = !entryHasOwnWindows(draft.models[modelId]) && tier === "peak" && wins.length > 0;
        const editing = winEdit !== null && winEdit.modelId === modelId && winEdit.tier === tier;
        const tag = (w, i) => el(
          "span",
          { key: tier + ":" + i, className: "dm-window-tag" + (inherited ? " inherited" : "") },
          formatWindow2(w),
          !inherited && el("button", {
            type: "button",
        ...
        return el(
          ...
          inherited && el("span", { className: "dm-window-inherit-note", title: t("windowsInheritedHint") }, t("windowsInherited")),
          wins.map(tag),
          !editing && el("button", {
            type: "button",
        ...
        el(
            ...
            return el(
              ...
              el(
                ...
                el(
                  ...
                  el("span", { className: "dm-price-name-text" }, modelId),
                  el("span", { className: "dm-price-base" }, t("basePrice"))
                ),
                el(
                  "span",
                  { className: "dm-price-actions" },
                  // 峰谷开关只留滑块(32px 轨):文字靠 title 提示,下方 空闲/高峰 行已标档位;
                  // 省下文字宽度给模型名,避免「模型名+基础价」与开关文字挤在一起盖住名字。
                  el(
                    "label",
                    { className: "dm-switch", title: t("tiersToggle") },
          ...
          el(
            ...
            el(
              "div",
              { className: "dm-price-fields" },
              el("span", { className: "dm-price-name" }, el("span", { className: "dm-price-name-text" }, t("defaultModel"))),
              el("span", { "aria-hidden": "true" }),
...
var require_dashboard = __commonJS({
  "lib/client-src/dashboard.js"(exports2, module2) {
    ...
    var { makeT: makeT3 } = (init_i18n(), __toCommonJS(i18n_exports));
    ...
    function Dashboard2({ wide, useMonitor, api, locale: baseLocale }) {
      const storeSnap = useMonitor ? useMonitor((s) => s) : {};
      const config = storeSnap.config;
      const locale = storeSnap.locale ?? baseLocale;
      const t = makeT3(locale);
      const decimals = Math.max(0, Math.min(10, Math.floor(Number(config?.decimals) || 4)));
      const cny = activeCurrency2(locale) === "cny";
  ...
  "lib/client-src/binding.js"(exports2, module2) {
    ...
    function startBinding2(ctx, deps) {
      ...
      const parseEditLabel = (label) => {
        ...
        if (!/^[A-Za-z0-9][A-Za-z0-9_.:@/\-]*$/.test(rest)) return null;
...
// lib/provider-alias.js
var provider_alias_exports = {};
__export(provider_alias_exports, {
  baseProviderId: () => baseProviderId
});
function baseProviderId(providerId) {
...
var { baseProviderId: baseProviderId2 } = (init_provider_alias(), __toCommonJS(provider_alias_exports));
injectStyles2();
...
async function apply(ctx) {
  ...
  const providerOf = (sessionId) => {
    ...
    try {
      ...
      const raw = snapshot?.current?.provider;
      const configured = store.getSnapshot().config?.providers?.[raw] !== void 0;
      return configured ? raw : baseProviderId2(raw);
    } catch {
      return void 0;
    }
  };
  ...
  slots.inject("conversation.input.right", () => slots.register({
    ...
    id: "dsh-monitor-icon",
    order: 0,
    inject: (sessionId) => ({ sessionId, api, providerOf, hooks: { monitor: store } })
  }, UsageButton));
  ...
  slots.inject("sidebar.footer.action", () => slots.register({
    ...
    order: 20,
    inject: () => ({ api, locale: store.getSnapshot().locale, hooks: { monitor: store } })
  }, Dashboard));
