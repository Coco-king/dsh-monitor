...
function createService(ctx, ledger) {
  ...
  const ensureUsage = async (providerId, force = false) => {
    ...
    if (provider === undefined) {
      return emptyUsage(providerId, undefined, 'off', tmsg(locale, 'providerNotConfigured'))
    }
    if (provider.enabled === false) {
      return emptyUsage(providerId, provider, 'off', tmsg(locale, 'providerDisabled'))
    }
    const cache = usageCaches.get(providerId)
