...
/**
 * 首次全量同步的进度报告器:在控制台提示用户同步正在进行(而非卡死)。
 * TTY 下用 `\r` 单行重绘进度条;非 TTY(日志重定向)下每 20% 打一条分步日志。
 * print/line 可注入,便于单测。
 * @param opts.total - 待扫描会话总数。
 */
export function createSweepReporter({ total, isTTY, print = s => process.stderr.write(s), line = s => console.log(s) } = {}) {
  const tty = isTTY ?? !!process.stderr?.isTTY
  const width = 24
  let lastPct = -20
  return {
    start() {
      line(`[dsh-monitor] 检测到 ${total} 个会话待全量同步，后台执行中（页面可正常使用）…`)
    },
    update(done) {
      const pct = total <= 0 ? 100 : Math.floor((done / total) * 100)
      if (tty) {
        const filled = Math.round((width * pct) / 100)
        print(`\r[dsh-monitor] 全量同步 [${'█'.repeat(filled)}${'░'.repeat(width - filled)}] ${String(done).padStart(String(total).length)}/${total} (${pct}%) `)
      } else if (pct - lastPct >= 20 || pct === 100) {
        lastPct = pct
        line(`[dsh-monitor] 全量同步： ${done}/${total} (${pct}%)`)
      }
    },
    finish(updated, elapsedMs) {
      if (tty) print(`\r${' '.repeat(72)}\r`)
      line(`[dsh-monitor] 全量同步完成：已扫描 ${total} 个会话，回填/更新 ${updated} 个，用时 ${(elapsedMs / 1000).toFixed(1)}s — 页面可正常查看用量`)
    },
  }
}
...
export function apply(ctx) {
  ...
  // 任一会话失败只记日志,绝不拖垮宿主。
  // 首轮全量同步性能:readFrom 是宿主 IO,按批并发重叠;批间让出事件循环,
  // 页面请求可插队(不卡);旧行清理由整轮结束的一次 prune 承担。
  ...
  const runSweep = async () => {
    ...
    try {
      ...
      const all = (await persistence.listSnapshots()) ?? []
      const total = all.length
      // 首次全量/补扫(还有会话没扫到进度)时在控制台报进度;全量结束后增量轮次静默。
      const startedAt = performance.now()
      const reporter = total > 0 && ledger.allProgress().length < total
        ? createSweepReporter({ total })
        : null
      if (reporter !== null) reporter.start()
      let scanned = 0
      let updated = 0
      // 处理单个会话:项目回填 → 未变跳过 → 增量折叠。返回 { scanned, updated }。
      const sweepOne = async snapshot => {
        const sessionId = snapshot?.header?.id ?? snapshot?.id
        if (sessionId === undefined) return { scanned: 0, updated: 0 }
        try {
          ...
          if (progress !== undefined && revision !== undefined && progress.logRevision === revision) {
      ...
      // 按批并发:批内 readFrom 重叠(缩短首轮全量同步墙钟),批间让出事件循环。
      for (let i = 0; i < total; i += SWEEP_CONCURRENCY) {
        const chunk = all.slice(i, i + SWEEP_CONCURRENCY)
        const results = await Promise.all(chunk.map(sweepOne))
        for (const result of results) {
          scanned += result.scanned
          updated += result.updated
        }
        reporter?.update(scanned)
        if (i + SWEEP_CONCURRENCY < total) {
          await new Promise(resolve => setImmediate(resolve))
        }
      }
      // 整轮结束后统一剪枝(只删超出保留天数的旧行;记录在 fold 事务内,不受影响)。
      ledger.prune()
      lastSweepAt = Date.now()
      if (reporter !== null) reporter.finish(updated, performance.now() - startedAt)
