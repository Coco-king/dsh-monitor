...
const windowSchema = z.object({ start: num, end: num })
...
const priceSchema = z.object({
  ...
  windows: z.object({
    peak: z.array(windowSchema).optional(),
    offPeak: z.array(windowSchema).optional(),
  }).optional(),
})
...
// ── getUsage(用量汇总:统计卡/提供方分布/项目分布/活跃度/模型表/页脚,费用双币) ──
...
const usageDaySchema = usageTotalsSchema.extend({ date: z.string() })
const usageSessionSchema = usageTotalsSchema.extend({ id: z.string(), date: z.string() })
const usageModelSchema = usageTotalsSchema.extend({ provider: z.string(), model: z.string() })
const usageProviderSchema = usageTotalsSchema.extend({ provider: z.string() })
const usageProjectSchema = usageTotalsSchema.extend({ project: z.string() })
const usageRouteSchema = usageTotalsSchema.extend({ day: z.string(), provider: z.string(), model: z.string() })
const providerNameSchema = z.object({ id: z.string(), name: z.string() })
...
const usageSummarySchema = z.object({
  ...
  byProvider: z.array(usageProviderSchema),
  byProject: z.array(usageProjectSchema),
  activity: z.array(usageDaySchema),
  activityModels: z.array(usageRouteSchema),
  windows: z.object({ today: usageTotalsSchema, month: usageTotalsSchema, all: usageTotalsSchema }),
  timeZone: z.object({ offset: z.string(), name: z.string().optional() }),
  lastSweepAt: z.number().nullable(),
  diagnostics: z.object({ lastUsageAt: z.number().nullable(), unattributedRows: z.number() }),
...
const _usageSummary$codec = { mode: 'strict', typeSymbol: 'dsh-monitor#UsageSummary', schema: usageSummarySchema }
const _usageQuery$codec = { mode: 'strict', typeSymbol: 'dsh-monitor#UsageQuery', schema: usageQuerySchema }
...
export const TYPERT = {
  ...
  invocations: [
    ...
    },
    {
      ...
      invocation: { kind: 'direct' },
      parameters: [
        { name: 'query', wire: 'query', source: 'json', codec: _usageQuery$codec },
      ],
      result: _usageSummary$codec,
    },
    {
    ...
    services: [
      {
        ...
        members: [
          ...
          },
          {
            kind: 'method',
            name: 'getUsage',
            signature: 'getUsage(query: UsageQuery): UsageSummary',
            summary: '用量汇总:按天/token/调用次数/费用(供柱形图、卡片、会话列表)。Usage summary grouped by day with cost, tokens and calls (for charts, cards, session list).',
            jsDoc: '/**\n * 用量汇总:按天产出 token 与费用,按提供方/模型/时间范围筛选。\n * @param query - { range, providers, models }。\n * @returns 总计、按天序列与会话列表。\n * Usage summary: aggregate tokens and cost by day, filtered by provider/model/time range.\n * @param query - { range, providers, models }.\n * @returns Totals, per-day series and session list.\n */',
          },
          {
            kind: 'method',
            name: 'getConfig',
