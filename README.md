...
1. **配置提供方**：设置 → 用量 → 提供方用量配置 → 添加提供方。**提供方 ID 用选择器**——候选 = 设置→模型 已配置的提供方 + 本插件已配置的提供方（含自定义），也可手输自定义 id。选择预设并填写字段后保存。**DeepSeek 官方为内置，无需配置**——只有 OpenCode、自定义或想自定义 DeepSeek 刷新间隔时才需要添加。
2. **查看用量**：输入栏右侧（模型切换器左侧）点击用量图标，面板展示当前会话提供方的额度；头部显示提供方名字与预设徽标，右上角刷新图标强制刷新。
3. **会话费用**：会话头部显示本会话费用 chip，悬停查看输入/缓存/输出 token 明细。
4. **价格**：设置 → 用量 → 计费价格，可手动编辑价格、**用按提供方分组的下拉选择器从 设置→模型 新增模型**（也可手输 ID）、或从官方文档同步。
